...
static int init_pixel_stream(void* user_struct, struct stream_state* stream)
{
    ...
    if (init_pixel_stream_hook(user_struct, stream)) return 1;
    stream->row = NULL;
...
static void free_blobs(struct blob_list* blist)
{
    if (blist->head != NULL) free(blist->head);
    if (blist->empties != NULL) free(blist->empties);
}
...
// (don't use on head, am lazy)
{
    ...
    b1 = b2->prev;
    b3 = b2->next;
    if (b1 != NULL) b1->next = b3;
    if (b3 != NULL) b3->prev = b1;
    b2->prev = NULL;
    b2->next = NULL;
    // unlink sibs
    s1 = b2->sib_p;
    s3 = b2->sib_n;
    if (s1 != NULL) s1->sib_n = s3;
    if (s3 != NULL) s3->sib_p = s1;
    b2->sib_p = NULL;
    b2->sib_n = NULL;
}
...
// must call next_row() to continue scanning
{
    ...
    for (;;) {          // awkward, but the two exit points are too similar
        ...
        if (b->color != (stream->row[stream->x])) break;
        stream->x++;
    }
    b->x2 = stream->x - 1;
    return 0;
...
{
    if (b->bb_x1 < 0) b->bb_x1 = x1;
    if (x1 < b->bb_x1) b->bb_x1 = x1;
    if (x2 > b->bb_x2) b->bb_x2 = x2;

    if (b->bb_y1 < 0) b->bb_y1 = y1;
    if (y1 < b->bb_y1) b->bb_y1 = y1;
...
// merge sort two sibling chains into one
{
    ...
    while (s1->sib_p != NULL) s1 = s1->sib_p;
    while (s2->sib_p != NULL) s2 = s2->sib_p;
    if (s1 == s2) return;
...
// pads by one for diagonals
{
    ...
    // b1 <= a1 <= b2
    if ((b1-1) <= a1 && a1 <= (b2+1)) return 1;
    // b1 <= a2 <= b2
    if ((b1-1) <= a2 && a2 <= (b2+1)) return 1;
    // a1 <= b1 <= a2
    if ((a1-1) <= b1 && b1 <= (a2+1)) return 1;
    // a1 <= b2 <= a2
    if ((a1-1) <= b2 && b2 <= (a2+1)) return 1;
    return 0;
}
...
// seems to do too much
{
    struct blob* s1;
    struct blob* s3;
    if (b->sib_p == NULL && b->sib_n == NULL) return;
    // paranoid...
...
// merges/prints/reaps everything before blob_now
{
    ...
    while (b->sib_p) {
        b = b->sib_p;
        if (b->y != blob_now->y-1) continue;
        if (b->x1 == -1) break;
        if (b->x2 > blob_now->x2) continue;
        // merge b into b2
        b2 = b->sib_n;
        blob_merge(b2, b);
        blob_reap(blist, b);
...
int extract_image(void* user_struct)
{
    ...
    if (init_pixel_stream(user_struct, &stream)) {
        LOGE("init malloc error!");
        return 1;
    }
    if (stream.row == NULL) {
        LOGE("row malloc error!");
        return 1;
    }
    blist.length = stream.w + 5;
    if (malloc_blobs(&blist)) {
        LOGE("blob malloc error!");
        return 1;
    }
