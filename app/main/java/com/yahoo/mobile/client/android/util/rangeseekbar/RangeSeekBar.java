...
//        }
    ...
    public void setSingleThumb(boolean single) {
        mSingleThumb = single;
    }
    ...
    private void trackTouchEvent(MotionEvent event) {
        ...
        if (mSingleThumb) {
            if (mReversed) {
                if (Thumb.MIN.equals(pressedThumb)) setNormalizedMinValue(screenToNormalized(x));
            }
            else {
                if (Thumb.MAX.equals(pressedThumb)) setNormalizedMaxValue(screenToNormalized(x));
            }
        }
        else {
            if (Thumb.MAX.equals(pressedThumb))
                setNormalizedMaxValue(screenToNormalized(x));
            else if (Thumb.MIN.equals(pressedThumb))
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        String minLabel = getAbsoluteMinValue() + " " + mUnits;
        String maxLabel = getAbsoluteMaxValue() + " " + mUnits;
        ...
        canvas.drawText(minLabel, padding - (INITIAL_PADDING + thumbHalfWidth + paint.measureText(minLabel)), minMaxHeight, paint);
        canvas.drawText(maxLabel, getWidth() - paint.measureText(maxLabel), minMaxHeight, paint);

        // draw seek bar background line
//        mRect.left = paint.measureText(minLabel) + thumbHalfWidth;
//        mRect.right = getWidth() - (paint.measureText(maxLabel) + thumbHalfWidth);
        mRect.left = padding;
...
//                getSelectedMaxValue().equals(getAbsoluteMaxValue()));
        ...
        if (!selectedValuesAreDefault) {
            ...
            String minText = String.valueOf(getSelectedMinValue()) + " " + mUnits;
            String maxText = String.valueOf(getSelectedMaxValue()) + " " + mUnits;
