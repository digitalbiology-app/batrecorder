...
/*
	if (r == 0 && S_ISDIR(statbuf.st_mode)) {
		...
		usbi_dbg("SYSFS_DEVICE_PATH=%s", SYSFS_DEVICE_PATH);
		if (!devices) {
		...
		while ((entry = readdir(devices))) {
			...
			// Only need to check until we've found ONE device which has all the attributes.
			if (sysfs_has_descriptors && sysfs_can_relate_devices)
				break;
		}
		closedir(devices);

		// Only use sysfs descriptors if the rest of sysfs will work for libusb.
...
static int find_fd_by_name(char *file_name)
{
	...
	while ((fd_dirent = readdir(proc_fd)) != 0)
	{
...
static int op_open2(struct libusb_device_handle *handle, int fd) {
	...
	if (r < 0) {
		if (errno == ENOTTY)
			...
			usbi_err(HANDLE_CTX(handle), "getcap failed (err=%d)", errno);
		hpriv->caps = 0;
		if (supports_flag_zero_packet)
	...
	int interface)
{
	...
	if (r) {
		...
		usbi_err(HANDLE_CTX(handle), "detach failed error %d errno %d", r, errno);
		return LIBUSB_ERROR_OTHER;
	}
	return 0;
}
	...
	struct usbfs_urb *urb)
{
	...
	default:
		usbi_warn(ITRANSFER_CTX(itransfer),
			"bulk_completion unrecognised urb status %d", urb->status);
		tpriv->reap_action = ERROR;
		goto cancel_remaining;
	}
	...
	struct usbfs_urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		default:
			usbi_warn(TRANSFER_CTX(transfer),
				"iso_completion packet %d unrecognised urb status %d", i, urb_desc->status);
			lib_desc->status = LIBUSB_TRANSFER_ERROR;
			break;
		}
	...
	default:
		usbi_warn(TRANSFER_CTX(transfer),
			"iso_completion unrecognised urb status %d", urb->status);
	...
	struct usbfs_urb *urb)
{
	...
	default:
		usbi_warn(ITRANSFER_CTX(itransfer),
			"control_completion unrecognised urb status %d", urb->status);
		status = LIBUSB_TRANSFER_ERROR;
		break;
	}
