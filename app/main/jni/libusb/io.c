...
int API_EXPORTED libusb_submit_transfer(struct libusb_transfer *transfer)
{
	...
	if (r < 0) {
		__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "calculate_timeout error");
		r = LIBUSB_ERROR_OTHER;
		goto out;
	}
	...
	if (r) {
		__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "submit_transfer error = %s", libusb_error_name(r));
		usbi_mutex_lock(&ctx->flying_transfers_lock);
